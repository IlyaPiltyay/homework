...
from src.external_api import (
    operation_code,
)  # Измените на актуальный путь к вашему модулю
...
def test_operation_code(mock_read_file, mock_requests_get):
    ...
    mock_transactions = [
        ...
        {"operationAmount": {"currency": {"code": "USD"}, "amount": "50.00"}},
        {"operationAmount": {"currency": {"code": "EUR"}, "amount": "20.00"}},
    ]
    ...
    expected_url = f"https://api.apilayer.com/exchangerates_data/convert?to=RUB&from=USD&amount={total_converted}"

    mock_requests_get.assert_called_once_with(
        expected_url, headers={"apikey": os.getenv("API_KEY")}
    )
